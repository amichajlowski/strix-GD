...
If an interactive audit is saved for later, resume it by run name:

```bash
strix --resume <run-name>
```

In the TUI, select a failed red agent and press `Esc` to retry it, save the run
for resume, or cancel while keeping findings.
